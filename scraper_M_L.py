...
# Download Image
def download_image(image_url, image_size, folder_path, cursor, conn, relation):
    ...
    if response.status_code == 200:
        ...
        with open(image_path, 'rb') as file:
            blob_data = file.read()
            table_name = "image_data_medium" if image_size == 'M' else "image_data_large"
            cursor.execute(f"""
...
def download_images(image_details, gender, product_info, option_id, cursor, conn, relation, base_folder_path='downloaded_images'):
    ...
    medium_folder_path = os.path.join(folder_structure, 'medium')
    large_folder_path = os.path.join(folder_structure, 'large')
    
    for medium_image_url, large_image_url in image_details:
        path_M = download_image(medium_image_url, 'M', medium_folder_path, cursor, conn, relation)
        path_L = download_image(large_image_url, 'L', large_folder_path, cursor, conn, relation)
...
def scrape_image_details(soup):
    ...
    if product_images_div:
        ...
        image_details = [(img['data-medium-image'], img['data-large-img-url']) for img in img_tags]
...
def create_database_tables(cursor):
    ...
    """)
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS image_data_medium (
            id SERIAL PRIMARY KEY,
            blob BYTEA,
            path VARCHAR,
            relation INTEGER
...
def main():
    ...
    def worker():
        ...
        while True:
            ...
            #nonlocal last_p_key, current_relation  # nonlocal anahtar kelimesi eklenmiştir
